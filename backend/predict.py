...
def predict(model, scaler, encoder, data):
    try:
        ...
        try:
            ...
            if 'rf'  in estimators: rf_label  = encoder.inverse_transform([estimators['rf'].predict(scaled)[0]])[0]
            if 'xgb' in estimators: xgb_label = encoder.inverse_transform([estimators['xgb'].predict(scaled)[0]])[0]
            if 'svm' in estimators: svm_label = encoder.inverse_transform([estimators['svm'].predict(scaled)[0]])[0]
        except Exception:
