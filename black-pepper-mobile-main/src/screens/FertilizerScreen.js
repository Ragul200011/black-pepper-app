...
const FERTS = [
  {
    id: 1, emoji: '🌿', name: 'Urea (46-0-0)',
    type: 'Nitrogen Booster', color: '#2e7d32', npk: '46-0-0',
  ...
  {
    id: 2, emoji: '⚖️', name: 'NPK 15-15-15',
    type: 'Balanced Complete', color: '#1565c0', npk: '15-15-15',
  ...
  {
    id: 3, emoji: '🌸', name: 'Single Super Phosphate',
    type: 'Phosphorus Booster', color: '#ad1457', npk: '0-16-0',
  ...
  {
    id: 4, emoji: '🟤', name: 'MOP — Muriate of Potash',
    type: 'Potassium Booster', color: '#6d4c41', npk: '0-0-60',
  ...
  {
    id: 5, emoji: '🪨', name: 'Agricultural Lime',
    type: 'pH Corrector', color: '#6a1b9a', npk: '0-0-0 + Ca + Mg',
  ...
  {
    id: 6, emoji: '🌱', name: 'Organic Compost',
    type: 'Soil Conditioner', color: '#4e342e', npk: '~1-1-1',
  ...
  {
    id: 7, emoji: '💎', name: 'NPK 12-32-16',
    type: 'Root & Bloom Blend', color: '#00838f', npk: '12-32-16',
...
function validateField(key, raw) {
  ...
  if (val < rule.min)        return `${rule.label} cannot be less than ${rule.min}${rule.unit ? ' ' + rule.unit : ''}.`;
  if (val > rule.max)        return `${rule.label} cannot exceed ${rule.max}${rule.unit ? ' ' + rule.unit : ''}.`;
  return null;
}
...
function localScore(id, n, p, k, ph) {
  ...
  if (id === 1) {
    s = 20;
    if (n < 20) s += 55; else if (n < 40) s += 40; else if (n < 60) s += 15; else if (n < 80) s += 2; else s -= 25;
    if (ph >= 5.5 && ph <= 7) s += 15; else if (ph < 5.5) s -= 10;
  } else if (id === 2) {
    s = 25;
    ...
    s = 18;
    if (p < 10) s += 58; else if (p < 20) s += 42; else if (p < 30) s += 20; else if (p < 40) s += 5; else s -= 15;
    if (ph >= 5.5 && ph <= 6.5) s += 14;
  } else if (id === 4) {
    s = 18;
    if (k < 30) s += 58; else if (k < 60) s += 42; else if (k < 90) s += 20; else if (k < 120) s += 5; else s -= 15;
    if (ph >= 5.5 && ph <= 7) s += 14;
  } else if (id === 5) {
    s = 15;
    if (ph < 4.5) s += 65; else if (ph < 5) s += 55; else if (ph < 5.5) s += 40; else if (ph < 6) s += 15; else if (ph > 7) s -= 25;
  } else if (id === 6) {
    s = 50;
...
function buildReason(id, n, p, k, ph) {
  ...
  if (id === 2) return `All nutrients moderate. NPK 15-15-15 prevents any single deficiency from developing.`;
  if (id === 3) return p < 10
  ...
  if (id === 6) return `Organic compost improves soil structure and helps all other fertilizers work more effectively.`;
  if (id === 7) return `High-P blend ideal for current soil profile — especially during root establishment.`;
  return '';
}
    ...
    >
      ...
      <View style={s.cardHead}>
        ...
        <View style={[s.matchTag, { backgroundColor: matchColor(sc) + '22' }]}>
          <Text style={[s.matchTxt, { color: matchColor(sc) }]}>
            {matchLabel(sc)} Match
          </Text>
        </View>
      </View>
      ...
      <View style={s.cardMain}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.cType}>{item.type} · NPK {item.npk}</Text>
        </View>
  ...
  const analyse = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${FERTILIZER_URL}?n=${nv}&p=${pv}&k=${kv}&ph=${phv}`,
        { timeout: 6000 }
      );
      ...
      >
        ...
        <View style={s.inputCard}>
          ...
          <View style={s.inputRow}>
            <NumericInput
              label="Nitrogen"   unit="mg/kg" value={n}  onChange={(v) => { setN(v);  setErrors((e) => ({ ...e, n:  null })); }}
              color="#2e7d32" hint="e.g. 45" error={errors.n}
            />
            <NumericInput
              label="Phosphorus" unit="mg/kg" value={p}  onChange={(v) => { setP(v);  setErrors((e) => ({ ...e, p:  null })); }}
              color="#ad1457" hint="e.g. 25" error={errors.p}
            />
          </View>

          <View style={s.inputRow}>
            <NumericInput
              label="Potassium"  unit="mg/kg" value={k}  onChange={(v) => { setK(v);  setErrors((e) => ({ ...e, k:  null })); }}
              color="#6d4c41" hint="e.g. 80" error={errors.k}
            />
            <NumericInput
              label="Soil pH"    unit=""      value={ph} onChange={(v) => { setPh(v); setErrors((e) => ({ ...e, ph: null })); }}
              color="#6a1b9a" hint="e.g. 6.2" error={errors.ph}
            />
          </View>
        ...
        {results && (
          <View style={s.results}>
            ...
            <View
              style={[
                s.sourcePill,
                { backgroundColor: source === 'api' ? '#e8f5e9' : '#fff8e1' },
              ]}
            >
              <Text
                style={{
            ...
            >
              ...
              <View style={s.topRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[s.topName, { color: results[0].color }]}>
                    {results[0].name}
                  </Text>
...
const s = StyleSheet.create({
  ...
  inputCard: {
    ...
    borderRadius: 22, padding: 20, elevation: 6,
    shadowColor: '#2d5016', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.12, shadowRadius: 16, marginBottom: 14,
  },
  ...
  reasonBox:   { borderLeftWidth: 3, padding: 12, backgroundColor: '#fafafa', borderRadius: 8, marginBottom: 12 },
  ...
  backBtn:     { borderRadius: 12, borderWidth: 1.5, borderColor: '#c8e6a0', padding: 13, alignItems: 'center', backgroundColor: '#fff', marginBottom: 12, marginTop: 6 },
  backBtnTxt:  { color: '#2d5016', fontSize: 14, fontWeight: '700' },
});

const ni = StyleSheet.create({
  label:    { fontSize: 11, fontWeight: '700', marginBottom: 6 },
  input:    { borderWidth: 1.5, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 11, fontSize: 15, color: '#222', backgroundColor: '#fafafa' },
