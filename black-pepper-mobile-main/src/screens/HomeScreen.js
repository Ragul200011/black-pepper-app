...
const FEATURES = [
  {
    ...
    desc: 'EfficientNetB0 detects leaf blight & slow wilt from leaf photos with 95%+ accuracy.',
    color: '#D32F2F',
    screen: 'DiseaseIdentification',
  },
  {
    ...
    desc: 'Two-stage AI classifies Butawerala, Dingirala & Kohukuburerala from leaf images.',
    color: C.teal,
    screen: 'VarietyHub',
  },
  {
    ...
    desc: 'Live NPK, pH, moisture & temperature from RS-485 IoT sensors via ThingSpeak cloud.',
    color: C.blue,
    screen: 'SoilAnalysis',
  },
  {
    ...
    desc: 'Ensemble RF+XGBoost+SVM ranks all 7 fertilizers by match percentage for your soil.',
    color: C.warning,
    screen: 'Fertilizer',
  },
  {
    ...
    desc: 'GPS satellite map with field markers, soil overlays and nearby farming spots.',
    color: C.rose,
    screen: 'Dashboard',
  },
  {
    ...
    desc: 'Live OpenWeatherMap data with auto-refreshing black pepper farming advisories.',
    color: C.indigo,
    screen: 'Weather',
  },
];
...
export default function HomeScreen({ navigation }) {
  ...
  // fade/rise are Animated.Value refs and intentionally stable
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.section}>
          ...
          {FEATURES.map((f) => (
            <TouchableOpacity
              key={f.title}
              style={[s.featCard, { borderLeftColor: f.color }]}
          ...
          {[
            {
              ...
              emoji: '📸',
              t: 'Capture / Upload',
              d: 'Take a clear photo of a leaf or enter sensor readings',
            },
            {
              n: '2',
              emoji: '🤖',
              t: 'AI Processing',
              d: 'Deep learning models analyse your image or data',
            },
            {
              n: '3',
              emoji: '📊',
              t: 'View Results',
              d: 'Disease, variety, soil verdict & recommendations',
            },
            {
              n: '4',
              emoji: '🌿',
              t: 'Take Action',
              d: 'Apply the fertilizer plan or treatment advice',
            },
          ].map((step) => (
            <View key={step.n} style={s.stepRow}>
              <View style={s.stepNum}>
                <Text style={s.stepNumTxt}>{step.n}</Text>
              </View>
