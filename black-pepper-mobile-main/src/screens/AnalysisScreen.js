...
const FARMS = ['SLIIT Research Plot','Engineering Faculty Field','Campus Garden','South Research Farm'];
...
function BarRow({ label, value, max, color, unit = '' }) {
  ...
  return (
    <View style={ba.row}>
      <Text style={ba.label} numberOfLines={1}>{label}</Text>
      <View style={ba.track}>
        <View style={[ba.fill, { width: `${pct}%`, backgroundColor: color }]} />
      </View>
      <Text style={[ba.val, { color }]}>{value}{unit}</Text>
    </View>
  );
}
...
export default function AnalyticsScreen({ navigation }) {
  ...
  const todayScans    = [...diseaseHistory, ...varietyHistory].filter(s => isToday(s.timestamp)).length;
  const weekScans     = [...diseaseHistory, ...varietyHistory].filter(s => isThisWeek(s.timestamp)).length;

  // Disease breakdown
  const diseaseBreakdown = {};
  let   diseaseTotalConf = 0, diseaseConfCount = 0;
  diseaseHistory.forEach(s => {
    ...
    if (s.confidence) { diseaseTotalConf += parseFloat(s.confidence); diseaseConfCount++; }
  });
  const avgDiseaseConf = diseaseConfCount > 0 ? (diseaseTotalConf / diseaseConfCount).toFixed(1) : '—';

  // Variety breakdown
  const varietyBreakdown = {};
  let   varTotalConf = 0, varConfCount = 0;
  varietyHistory.forEach(s => {
    const key = s.result ?? 'Unknown';
    varietyBreakdown[key] = (varietyBreakdown[key] ?? 0) + 1;
    if (s.confidence) { varTotalConf += parseFloat(s.confidence); varConfCount++; }
  });
  ...
  const allConfs = [...diseaseHistory, ...varietyHistory]
    .map(s => parseFloat(s.confidence)).filter(v => !isNaN(v));
  ...
  const summaryCards = [
    { icon:'camera-outline',    color:'#1565C0', bg:'#E3F2FD', label:'Total Scans',   val:totalScans   },
    { icon:'today-outline',     color:'#2E7D32', bg:'#E8F5E9', label:'Today',         val:todayScans   },
    { icon:'calendar-outline',  color:'#6A1B9A', bg:'#F3E5F5', label:'This Week',     val:weekScans    },
    { icon:'bug-outline',       color:'#C62828', bg:'#FFEBEE', label:'Disease Scans', val:diseaseHistory.length },
    { icon:'leaf-outline',      color:'#2E7D32', bg:'#E8F5E9', label:'Variety Scans', val:varietyHistory.length },
    { icon:'stats-chart-outline',color:'#E65100',bg:'#FFF3E0', label:'Avg Confidence',val:`${avgDiseaseConf}%`  },
  ];
  ...
  return (
    <ScrollView style={s.root} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.card}>
        ...
        <Text style={s.cardSub}>How certain the AI is across all {allConfs.length} predictions</Text>
        <View style={{ gap: 10, marginTop: 14 }}>
          {[
            { label:'High (≥75%)',    val:high,   color:'#2E7D32', pct:Math.round(high/total*100)   },
            { label:'Medium (55–74%)',val:medium, color:'#F57F17', pct:Math.round(medium/total*100) },
            { label:'Low (<55%)',     val:low,    color:'#C62828', pct:Math.round(low/total*100)    },
          ].map(row => (
        ...
        {allConfs.length === 0 && <Text style={s.noData}>No predictions yet. Run some scans first.</Text>}
      </View>
      ...
      <View style={s.card}>
        <View style={s.cardTitleRow}>
          ...
          <Text style={s.cardBadge}>{diseaseHistory.length} scans · avg {avgDiseaseConf}%</Text>
        </View>
      ...
      <View style={s.card}>
        <View style={s.cardTitleRow}>
          ...
          <Text style={s.cardBadge}>{varietyHistory.length} scans · avg {avgVarConf}%</Text>
        </View>
      ...
      <View style={s.card}>
        ...
        <View style={s.tableHeader}>
          {['Farm','N','P','K','pH','Status'].map(h => (
            <Text key={h} style={[s.tableH, h==='Farm' && {flex:2}]}>{h}</Text>
          ))}
        </View>
        {[
          ...
          <View key={farm.name} style={[s.tableRow, i%2===1 && s.tableRowAlt]}>
            <Text style={[s.tableCell, {flex:2}]} numberOfLines={1}>{farm.name}</Text>
      ...
      <View style={s.quickRow}>
        {[
          { label:'Disease Scan', icon:'bug-outline',   screen:'DiseaseIdentification', color:'#C62828' },
          { label:'Variety Scan', icon:'leaf-outline',  screen:'VarietyHub',            color:'#2E7D32' },
          { label:'Sensor Trends',icon:'trending-up-outline',screen:'SensorTrend',      color:'#1565C0' },
        ].map(q => (
          <TouchableOpacity key={q.screen} style={s.quickBtn} onPress={() => navigation.navigate(q.screen)} activeOpacity={0.8}>
...
const s = StyleSheet.create({
  ...
  heroBadge:    { flexDirection:'row', alignItems:'center', backgroundColor:'rgba(255,255,255,0.15)', borderRadius:20, paddingHorizontal:12, paddingVertical:5, marginBottom:12, alignSelf:'flex-start' },
  ...
  summaryCard:  { flex:1, minWidth:'28%', backgroundColor:C.surface, borderRadius:16, padding:14, alignItems:'center', ...SHADOW.sm, borderWidth:1, borderColor:C.border },
  ...
  card:         { marginHorizontal:16, marginBottom:14, backgroundColor:C.surface, borderRadius:20, padding:18, ...SHADOW.sm, borderWidth:1, borderColor:C.border },
  ...
  confTrack: { flex:1, height:10, backgroundColor:C.surface2, borderRadius:5, overflow:'hidden' },
  ...
  tableHeader:  { flexDirection:'row', paddingVertical:8, borderBottomWidth:1, borderBottomColor:C.border, marginTop:12 },
  tableH:       { flex:1, fontSize:11, fontWeight:'800', color:C.text3, textTransform:'uppercase', textAlign:'center' },
  ...
  quickBtn: { flex:1, backgroundColor:C.surface, borderRadius:16, padding:14, alignItems:'center', ...SHADOW.sm, borderWidth:1, borderColor:C.border },
