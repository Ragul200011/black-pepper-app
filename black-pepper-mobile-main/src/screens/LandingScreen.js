...
const MODULES = [
  { emoji:'🌱', title:'Soil Monitor',       sub:'Live NPK · pH · Temperature via ThingSpeak IoT', screen:'SoilAnalysis',          color:C.blue,    bg:'#E3F2FD' },
  { emoji:'🔬', title:'Disease Detection',  sub:'Leaf blight & slow wilt · EfficientNetB0 AI',    screen:'DiseaseIdentification', color:'#D32F2F', bg:'#FFEBEE' },
  { emoji:'🫑', title:'Variety Identifier', sub:'Butawerala · Dingirala · Kohukuburerala',        screen:'VarietyHub',            color:C.teal,    bg:'#E0F7FA' },
  { emoji:'🌿', title:'Fertilizer Advisor', sub:'AI-ranked NPK recommendations for your soil',   screen:'Fertilizer',            color:C.warning, bg:'#FFF8E1' },
  { emoji:'🗺️', title:'Farm Dashboard',    sub:'GPS satellite map · Field overview',             screen:'Dashboard',             color:C.rose,    bg:'#FCE4EC' },
  { emoji:'🌤️', title:'Weather Station',   sub:'Live weather · Farming advisories',              screen:'Weather',               color:C.indigo,  bg:'#E8EAF6' },
];
            ...
            <Animated.View style={{ opacity:fade, transform:[{translateY:rise}] }}>
              ...
              <View style={s.topBar}>
                <View style={s.logoRow}>
                  <LinearGradient colors={['rgba(255,255,255,0.25)','rgba(255,255,255,0.1)']} style={s.logoIcon}>
              ...
              <View style={s.heroBody}>
                ...
                <View style={s.statsRow}>
                  {STATS.map(({ val, lbl, icon }) => (
                    <View key={lbl} style={s.statBox}>
                      <Ionicons name={icon} size={16} color="rgba(255,255,255,0.85)" style={{marginBottom:4}} />
                ...
                {!currentUser && (
                  <View style={s.ctaRow}>
                    <TouchableOpacity style={s.ctaPrimary} onPress={() => navigation.navigate('SignUp')} activeOpacity={0.88}>
                      <Text style={s.ctaPrimaryTxt}>Get Started Free</Text>
                      <Ionicons name="arrow-forward" size={16} color={C.primary} />
                    </TouchableOpacity>
                    <TouchableOpacity style={s.ctaSecondary} onPress={() => navigation.replace('Home')} activeOpacity={0.88}>
                      <Text style={s.ctaSecondaryTxt}>Browse as Guest</Text>
                    </TouchableOpacity>
                  </View>
                )}
                {currentUser && (
                  <TouchableOpacity style={s.ctaPrimary} onPress={() => navigation.replace('Home')} activeOpacity={0.88}>
          ...
          <View style={s.moduleGrid}>
            {MODULES.map((m, i) => (
              <Animated.View key={m.screen}
                ...
                <TouchableOpacity style={[s.moduleCard, {borderTopColor:m.color}]}
                  activeOpacity={0.8} onPress={() => navigation.navigate(m.screen)}
            ...
            <View style={s.aboutRow}>
              <View style={{flex:1}}>
                ...
                <Text style={s.aboutDesc}>
                  Developed to improve black pepper cultivation in Sri Lanka using deep learning, IoT sensors, and mobile technology.
                </Text>
...
const s = StyleSheet.create({
  ...
  topBar:  { flexDirection:'row', alignItems:'center', justifyContent:'space-between', paddingHorizontal:20, paddingTop:10, marginBottom:28 },
  ...
  btnOutlineW:   { paddingHorizontal:14, paddingVertical:8, borderRadius:20, borderWidth:1.5, borderColor:'rgba(255,255,255,0.55)' },
  btnOutlineWTxt:{ color:C.white, fontSize:12, fontWeight:'700' },
  btnSolidW:     { paddingHorizontal:14, paddingVertical:8, borderRadius:20, backgroundColor:'rgba(255,255,255,0.22)' },
  btnSolidWTxt:  { color:C.white, fontSize:12, fontWeight:'800' },

  userBadge:    { flexDirection:'row', alignItems:'center', gap:6, backgroundColor:'rgba(255,255,255,0.2)', paddingHorizontal:12, paddingVertical:8, borderRadius:20 },
  ...
  heroBadgeTxt:{ fontSize:10, color:'rgba(255,255,255,0.85)', fontWeight:'800', letterSpacing:2 },
  heroTitle:   { fontSize:32, fontWeight:'900', color:C.white, lineHeight:39, marginBottom:12, letterSpacing:-0.5 },
  heroDesc:    { fontSize:13, color:'rgba(255,255,255,0.82)', lineHeight:20, marginBottom:22 },

  statsRow: { flexDirection:'row', gap:8, marginBottom:24 },
  statBox:  { flex:1, backgroundColor:'rgba(255,255,255,0.14)', borderRadius:16, padding:12, alignItems:'center', borderWidth:1, borderColor:'rgba(255,255,255,0.15)' },
  ...
  ctaPrimary:   { flexDirection:'row', alignItems:'center', gap:8, backgroundColor:C.white, borderRadius:14, paddingVertical:13, paddingHorizontal:20, ...SHADOW.md },
  ctaPrimaryTxt:{ color:C.primary, fontSize:14, fontWeight:'900' },
  ctaSecondary: { borderRadius:14, paddingVertical:13, paddingHorizontal:18, borderWidth:1.5, borderColor:'rgba(255,255,255,0.45)', justifyContent:'center' },
  ...
  sectionTitle:   { fontSize:22, fontWeight:'900', color:C.text, marginBottom:6, textAlign:'center' },
  ...
  moduleCard: { backgroundColor:C.white, borderRadius:18, padding:16, borderTopWidth:3, borderWidth:1, borderColor:C.border, ...SHADOW.sm, marginBottom:0 },
  moduleEmoji:{ width:52, height:52, borderRadius:16, justifyContent:'center', alignItems:'center', marginBottom:12 },
  moduleTitle:{ fontSize:14, fontWeight:'900', marginBottom:4 },
  moduleSub:  { fontSize:11, color:C.text3, lineHeight:15, marginBottom:12 },
  moduleChip: { alignSelf:'flex-start', paddingHorizontal:10, paddingVertical:5, borderRadius:20 },
  ...
  aboutTag:     { backgroundColor:'rgba(255,255,255,0.2)', borderRadius:20, paddingHorizontal:10, paddingVertical:4 },
