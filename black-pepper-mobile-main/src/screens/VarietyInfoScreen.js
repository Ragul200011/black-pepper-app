...
const VARIETY_DATA = {
  Butawerala: {
    ...
    tips: [
      'Space vines 2–3 m apart',
      'Apply NPK 15-15-15 every 3 months',
      'Harvest when 5–10% berries turn red',
      'Prune after each harvest season',
    ],
  },
  Dingirala: {
    color: '#1565C0',
    ...
    tips: [
      'Ideal for shaded cultivation',
      'Mulch heavily to retain moisture',
      'Harvest before full maturity for maximum aroma',
  ...
  Kohukuburerala: {
    ...
    tips: [
      'Requires sturdy support poles',
      'Water regularly during dry season',
      'Apply potassium-rich fertilizer at flowering',
      'Best suited for highland regions',
    ],
  },
};
...
export default function VarietyInfoScreen({ route, navigation }) {
  ...
  return (
    ...
    >
      {/* Hero */}
      <LinearGradient
        colors={[data.color, data.color + 'CC', data.color + '44', C.bg]}
        style={s.hero}
      >
        <Text style={s.heroEmoji}>{data.emoji}</Text>
        <Text style={s.heroName}>{varietyName}</Text>
        <Text style={s.heroOrigin}>
          <Ionicons name="location-outline" size={12} /> {data.origin}
        </Text>
        {result && (
