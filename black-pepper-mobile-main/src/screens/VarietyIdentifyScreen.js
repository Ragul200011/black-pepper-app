...
export default function VarietyIdentifyScreen({ navigation }) {
  ...
  const requestPermission = useCallback(async (type) => {
    ...
    if (!perm.granted) {
      Alert.alert('Permission Required', `${type === 'camera' ? 'Camera' : 'Gallery'} permission is required.`);
      return false;
    }
    return true;
  }, []);
  ...
  const pickFromGallery = useCallback(async () => {
    ...
    try {
      ...
      if (!res.canceled && res.assets?.length > 0) { setAsset(res.assets[0]); setError(''); }
  ...
  const takePhoto = useCallback(async () => {
    ...
    try {
      const res = await ImagePicker.launchCameraAsync({ mediaTypes:['images'], allowsEditing:true, aspect:[1,1], quality:0.85 });
      if (!res.canceled && res.assets?.length > 0) { setAsset(res.assets[0]); setError(''); }
  ...
  const saveToHistory = useCallback(async (data, uri) => {
    try {
      ...
      history.unshift({ id:Date.now().toString(), imageUri: Platform.OS==='web'&&uri?.startsWith('blob:') ? null : uri,
        result:data.result, confidence:data.confidence, stage:data.stage, timestamp:new Date().toISOString() });
  ...
  const handleReset = useCallback(() => { setAsset(null); setError(''); }, []);

  return (
    <View style={s.root}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        <LinearGradient colors={[C.gradStart, C.gradMid, C.gradEnd]} style={s.hero}>
          <View style={s.heroBadge}><View style={s.heroDot}/><Text style={s.heroBadgeTxt}>LEAF VARIETY DETECTION</Text></View>
          <Text style={s.heroTitle}>Identify Leaf Variety</Text>
          <Text style={s.heroSub}>Upload or capture a pepper leaf image to identify the variety using AI.</Text>
        </LinearGradient>
        ...
        <View style={s.card}>
          ...
          <View style={s.pickerRow}>
            <TouchableOpacity style={s.pickerBtn} onPress={pickFromGallery} activeOpacity={0.82}>
              <View style={[s.pickerIcon,{backgroundColor:'#E3F2FD'}]}><Ionicons name="images-outline" size={26} color={C.blue}/></View>
              <Text style={s.pickerBtnTxt}>Gallery</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.pickerBtn} onPress={takePhoto} activeOpacity={0.82}>
              <View style={[s.pickerIcon,{backgroundColor:C.xlight}]}><Ionicons name="camera-outline" size={26} color={C.primary}/></View>
          ...
          <TouchableOpacity style={[s.analyseBtn,(!asset||loading)&&{opacity:0.5}]}
            onPress={handlePredict} disabled={!asset||loading} activeOpacity={0.85}>
            {loading
              ? <><ActivityIndicator color={C.white} size="small"/><Text style={s.analyseBtnTxt}>  Analysing…</Text></>
        ...
        <View style={s.tipsCard}>
          ...
          {['Use a clear, well-lit close-up of a single leaf','Ensure the full leaf is visible','Avoid blurry or shadowed images'].map((tip,i)=>(
        ...
        <TouchableOpacity style={s.historyBtn} onPress={() => navigation.navigate('VarietyHistory')} activeOpacity={0.85}>
...
const s = StyleSheet.create({
  ...
  card:{margin:16,backgroundColor:C.white,borderRadius:20,padding:20,borderWidth:1,borderColor:C.border,...SHADOW.sm},
  ...
  emptyPreview:{height:160,borderRadius:14,backgroundColor:C.surface2,justifyContent:'center',alignItems:'center',gap:8,borderWidth:1.5,borderColor:C.border,borderStyle:'dashed',marginBottom:16},
  emptyTxt:{fontSize:13,color:C.hint},
  errBox:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'#FFF5F5',borderRadius:10,padding:12,marginBottom:12,borderWidth:1,borderColor:C.error+'33'},
  errTxt:{flex:1,fontSize:13,color:C.error,lineHeight:18},
  analyseBtn:{flexDirection:'row',justifyContent:'center',alignItems:'center',backgroundColor:C.primary,borderRadius:14,paddingVertical:16,...SHADOW.md},
  analyseBtnTxt:{color:C.white,fontSize:16,fontWeight:'800'},
  tipsCard:{marginHorizontal:16,backgroundColor:'#E8F5E9',borderRadius:16,padding:16,borderWidth:1,borderColor:C.light,marginBottom:12},
  ...
  historyBtn:{marginHorizontal:16,flexDirection:'row',alignItems:'center',gap:10,backgroundColor:C.white,borderRadius:14,padding:16,borderWidth:1,borderColor:C.border,...SHADOW.xs},
