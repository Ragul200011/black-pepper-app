...
export const SHADOW = {
  xs: Platform.select({
    ios:     { shadowColor:'#1A2E1A', shadowOffset:{width:0,height:1}, shadowOpacity:0.07, shadowRadius:4  },
    android: { elevation:1 },
    web:     { boxShadow:'0 1px 4px rgba(26,46,26,0.07)' },
  }),
  sm: Platform.select({
    ios:     { shadowColor:'#1A2E1A', shadowOffset:{width:0,height:2}, shadowOpacity:0.09, shadowRadius:8  },
    android: { elevation:2 },
    web:     { boxShadow:'0 2px 8px rgba(26,46,26,0.09)' },
  }),
  md: Platform.select({
    ios:     { shadowColor:'#1A2E1A', shadowOffset:{width:0,height:4}, shadowOpacity:0.11, shadowRadius:14 },
    android: { elevation:4 },
    web:     { boxShadow:'0 4px 16px rgba(26,46,26,0.11)' },
  }),
  lg: Platform.select({
    ios:     { shadowColor:'#1A2E1A', shadowOffset:{width:0,height:8}, shadowOpacity:0.13, shadowRadius:24 },
    android: { elevation:8 },
