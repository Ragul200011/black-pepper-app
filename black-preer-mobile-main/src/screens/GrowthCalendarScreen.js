...
import {
  ...
  Alert, TextInput, Modal, Platform, KeyboardAvoidingView,
} from 'react-native';
...
let Calendar;
try { Calendar = require('react-native-calendars').Calendar; }
catch { Calendar = null; }
...
export default function GrowthCalendarScreen({ navigation }) {
  ...
  const deleteEvent = (dateStr, eventId) => {
    ...
    if (Platform.OS === 'web') { if (window.confirm('Delete this event?')) doDelete(); return; }
        ...
        <View style={s.calCard}>
          {Calendar ? (
            ...
            <View style={s.noCalMsg}>
              <Ionicons name="calendar-outline" size={32} color={C.hint} style={{ marginBottom: 8 }} />
              ...
              return (
                <View key={ev.id} style={s.eventCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.eventTime}>{new Date(ev.createdAt).toLocaleTimeString([], { hour:'2-digit', minute:'2-digit' })}</Text>
                  </View>
                  <TouchableOpacity onPress={() => deleteEvent(selected, ev.id)} style={s.deleteBtn}>
              ...
              return (
                <View key={`${ev.id}-${i}`} style={s.timelineItem}>
                  ...
                  <View style={s.timelineContent}>
                    <Text style={s.timelineLabel}>{ev.label} {ev.farm ? `· ${ev.farm}` : ''}</Text>
          ...
          <View style={s.modalSheet}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.typeScroll}>
              {EVENT_TYPES.map(t => (
                <TouchableOpacity
                  key={t.id}
                  style={[s.typeChip, { backgroundColor: t.bg, borderColor: eventType.id === t.id ? t.color : 'transparent', borderWidth: 2 }]}
            ...
            <View style={s.modalBtns}>
              <TouchableOpacity style={s.modalCancelBtn} onPress={() => setShowModal(false)} activeOpacity={0.8}>
...
const s = StyleSheet.create({
  ...
  heroStats:    { flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: 14, padding: 12 },
  ...
  calCard: { backgroundColor: C.surface, margin: 16, borderRadius: 20, overflow: 'hidden', ...SHADOW.md, borderWidth: 1, borderColor: C.border, padding: 8 },
  noCalMsg:   { alignItems: 'center', padding: 24 },
  noCalTxt:   { fontSize: 14, fontWeight: '700', color: C.text, marginBottom: 6, textAlign: 'center' },
  noCalSub:   { fontSize: 12, color: C.info, marginBottom: 10 },
  noCalDate:  { fontSize: 13, color: C.text3 },

  addRow:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, marginBottom: 12 },
  addLabel:  { fontSize: 14, fontWeight: '700', color: C.text },
  addBtn:    { flexDirection: 'row', alignItems: 'center', backgroundColor: C.primary, paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, ...SHADOW.sm },
  addBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 13 },

  emptyDay:    { margin: 16, backgroundColor: C.surface, borderRadius: 14, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: C.border },
  emptyDayTxt: { fontSize: 13, color: C.text3, textAlign: 'center' },

  eventList: { paddingHorizontal: 16, gap: 10 },
  eventCard: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: C.surface, borderRadius: 16, padding: 14, gap: 12, ...SHADOW.xs, borderWidth: 1, borderColor: C.border, marginBottom: 8 },
  eventIcon: { width: 40, height: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  ...
  timelineSection: { margin: 16, backgroundColor: C.surface, borderRadius: 18, padding: 16, ...SHADOW.sm, borderWidth: 1, borderColor: C.border },
  timelineTitle:   { fontSize: 14, fontWeight: '800', color: C.text, marginBottom: 14, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  modalSheet:   { backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 24 },
  modalHandle:  { width: 40, height: 4, backgroundColor: C.border, borderRadius: 2, alignSelf: 'center', marginBottom: 18 },
  modalTitle:   { fontSize: 18, fontWeight: '800', color: C.text, marginBottom: 18 },
  modalLabel:   { fontSize: 12, fontWeight: '700', color: C.text3, marginBottom: 8, textTransform: 'uppercase' },
  typeScroll:   { marginBottom: 16 },
  typeChip:     { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, marginRight: 8 },
  typeChipTxt:  { fontSize: 13, fontWeight: '700' },
  modalInput:   { backgroundColor: C.surface2, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: C.text, borderWidth: 1, borderColor: C.border, marginBottom: 14 },
  modalTextArea:{ height: 80, textAlignVertical: 'top' },
  modalBtns:    { flexDirection: 'row', gap: 12, marginTop: 4 },
  modalCancelBtn:{ flex: 1, backgroundColor: C.surface2, borderRadius: 14, paddingVertical: 15, alignItems: 'center', borderWidth: 1, borderColor: C.border },
