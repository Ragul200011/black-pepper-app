  ...
  return (
    <TouchableOpacity style={s.card} onPress={onPress} activeOpacity={0.8}>
      ...
      <View style={s.cardContent}>
        <View style={[s.varietyBadge, { backgroundColor: varColor + '18', borderColor: varColor + '44' }]}>
      ...
      <View style={s.header}>
        ...
        <View style={s.headerContent}>
          <View style={s.headerLeft}>
            ...
            <Text style={s.headerSub}>{history.length} record{history.length !== 1 ? 's' : ''}</Text>
          </View>
          {history.length > 0 && (
...
const s = StyleSheet.create({
  ...
  header:        { backgroundColor:C.white, borderBottomWidth:1, borderBottomColor:C.border, ...SHADOW.sm },
  headerContent: { flexDirection:'row', alignItems:'center', justifyContent:'space-between', paddingHorizontal:16, paddingVertical:14 },
  headerLeft:    {},
  headerTitle:   { fontSize:20, fontWeight:'900', color:C.text },
  headerSub:     { fontSize:12, color:C.text3, marginTop:2 },
  clearBtn:      { flexDirection:'row', alignItems:'center', gap:5, backgroundColor:'#FFF5F5', borderRadius:10, paddingHorizontal:12, paddingVertical:7, borderWidth:1, borderColor:C.error+'33' },
  clearBtnTxt:   { color:C.error, fontSize:12, fontWeight:'700' },

  list:           { padding:16, paddingBottom:24 },
  emptyContainer: { flex:1, justifyContent:'center' },

  card:         { flexDirection:'row', alignItems:'center', backgroundColor:C.white, borderRadius:16, padding:12, marginBottom:10, borderWidth:1, borderColor:C.border, ...SHADOW.xs },
  thumb:        { width:64, height:64, borderRadius:12, backgroundColor:C.surface2 },
  thumbPlaceholder: { width:64, height:64, borderRadius:12, backgroundColor:C.surface2, justifyContent:'center', alignItems:'center' },
  cardContent:  { flex:1, marginLeft:12, gap:5 },
  chevron:      { marginLeft:4 },

  varietyBadge: { flexDirection:'row', alignItems:'center', gap:5, alignSelf:'flex-start', paddingHorizontal:10, paddingVertical:4, borderRadius:99, borderWidth:1, marginBottom:2 },
